...
> ⚠️ **核心原则：逐句翻译，一句不漏。**
>
> 原文的每一句话都必须在翻译中有对应的中文句子。禁止合并多句为一句，禁止用概括性语言替代原文，禁止跳过"不重要"的句子。翻译完成后，原文句子数量和翻译句子数量应大致相等。

1. **逐句忠实翻译** — 原文一句对应翻译一句，不重组、不省略、不总结、不跳过
2. **禁止概括** — 不允许出现"本节介绍了…"、"作者讨论了…"等概括性描述来替代实际翻译
3. 关键术语首次出现时标注英文原文：`潜在帧注入（latent frame injection）`
4. 公式保留原始 LaTeX：行内 `$x_0$`，块级 `$$\mathcal{L} = ...$$`
5. 图表描述翻译：`**图1：** {翻译图注}`
6. 每页用 `<!-- PAGE N -->` 标记，确保与原文 PDF 页码对齐
7. 批注穿插在翻译中，用 blockquote 格式
8. **第 1 页必须包含**：论文定位批注 + 作者阵容分析批注（从精炼版浓缩）
